...
Current candidate:
`git.home.corenode.com/homelab/sglang-glm53-flash-sm120-container:v0.1.0-rc.50`.
Local build name: `sglang-glm53-flash-sm120:v0.1.0-rc.50`.

**v0.1.0-rc.50 is a source candidate, not a qualified release.** Performance,
...
whole-layer ignore still selects BF16. Cache schema `v37` prevents reuse of
...
prenorm and fused post/pre paths. v0.1.0-rc.50 retains the same bounded
...
v0.1.0-rc.49 passed all five CPU ownership regressions, then stopped in the
new CUDA replay test because its asynchronous expected-value clone could race
replay on another stream. v0.1.0-rc.50 adds the missing test synchronization;
its runtime source is otherwise byte-for-byte identical.
...
and compiled DSA gate output is the v0.1.0-rc.50 runtime fix.
...
podman build \
  ...
  -t sglang-glm53-flash-sm120:v0.1.0-rc.50 .
```
