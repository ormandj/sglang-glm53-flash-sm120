...
check_pin GLM53_SGLANG_UPSTREAM_TREE "$(jq -er '.integration.sglang.upstream_tree' "$lock")"
check_pin GLM53_SGLANG_TREE "$(jq -er '.integration.sglang.tree' "$lock")"
check_pin GLM53_SGLANG_PATCH_SHA256 "$(jq -er '.integration.sglang.patch_sha256' "$lock")"
check_pin GLM53_FLASHINFER_HEAD "$(jq -er '.integration.flashinfer.head' "$lock")"
check_pin GLM53_FLASHINFER_UPSTREAM_TREE "$(jq -er '.integration.flashinfer.upstream_tree' "$lock")"
check_pin GLM53_FLASHINFER_TREE "$(jq -er '.integration.flashinfer.tree' "$lock")"
check_pin GLM53_FLASHINFER_PATCH_SHA256 "$(jq -er '.integration.flashinfer.patch_sha256' "$lock")"
