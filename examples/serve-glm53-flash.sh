...
# goals to qualify, not claims that v0.1.1-rc.12 has already achieved them.
set -euo pipefail
...
IMAGE=${IMAGE:-sglang-glm53-flash-sm120:v0.1.1-rc.12}
PORT=${PORT:-8000}
...
if [[ "$TP_SIZE" != 2 ]]; then
  echo "v0.1.1-rc.12 is scoped to TP_SIZE=2" >&2
  exit 2
fi
