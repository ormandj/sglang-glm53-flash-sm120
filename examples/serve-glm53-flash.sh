...
# goals to qualify, not claims that v0.1.0-rc.18 has already achieved them.
set -euo pipefail
...
IMAGE=${IMAGE:-sglang-glm53-flash-sm120:v0.1.0-rc.18}
PORT=${PORT:-8000}
...
if [[ "$TP_SIZE" != 2 ]]; then
  echo "v0.1.0-rc.18 is scoped to TP_SIZE=2" >&2
  exit 2
fi
